...
with tf.name_scope('hidden_weights_3'):
    # the weight matrix that maps the inputs to hiddden layer
    W3 = tf.Variable(np.random.normal(0, 0.05, (hiddenUnits, hiddenUnits)), dtype=tf.float32)

with tf.name_scope('hidden_biases_3'):
    # biaes for the hidden values
    b3 = tf.Variable(np.zeros((1, hiddenUnits)), dtype=tf.float32)
...
with tf.name_scope('final_weights'):
    ...
    W4 = tf.Variable(np.random.normal (0, 0.05, (hiddenUnits, numClasses)), dtype=tf.float32)
