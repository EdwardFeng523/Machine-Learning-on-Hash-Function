...
with tf.name_scope('final_weights'):
    ...
    W3 = tf.Variable(np.random.normal (0, 0.1, (hiddenUnits, numClasses)), dtype=tf.float32)
...
trainingAlg = tf.train.AdagradOptimizer(0.01).minimize(totalLoss)
...
with tf.Session() as sess:
    ...
    train_writer = tf.summary.FileWriter('./logs_sine_two_layer', sess.graph)
